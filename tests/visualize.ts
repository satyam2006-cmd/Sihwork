...
// ---------------------------------------------------------------------------
// ACI-BENCH configuration
// ---------------------------------------------------------------------------

const ACI_METRIC_LABELS: Record<string, string> = {
  aci_overall_rouge: 'Overall ROUGE',
  aci_subjective_rouge: 'Subjective',
  aci_objective_rouge: 'Objective',
  aci_assessment_rouge: 'Assessment',
  aci_plan_rouge: 'Plan',
};

const ACI_CATEGORIES: Record<string, string[]> = {
  'SOAP Section ROUGE Scores': [
    'aci_overall_rouge',
    'aci_subjective_rouge',
    'aci_objective_rouge',
    'aci_assessment_rouge',
    'aci_plan_rouge',
  ],
};
...
// ---------------------------------------------------------------------------
// ACI-BENCH results loading
// ---------------------------------------------------------------------------

function loadACIBenchResults(): ACIBenchResult[] {
  const args = process.argv.slice(2);
  const compareAll = args.includes('--compare');
  const specificFiles = args.filter(a => a.endsWith('.json'));

  if (!existsSync(RESULTS_DIR)) {
    console.error('No results directory found. Run `npm run test:aci-bench` first.');
    process.exit(1);
  }

  const allFiles = readdirSync(RESULTS_DIR)
    .filter(f => f.startsWith('aci-bench-') && f.endsWith('.json'))
    .sort();

  if (allFiles.length === 0) {
    console.error('No ACI-BENCH results found. Run `npm run test:aci-bench` first.');
    process.exit(1);
  }
  ...
  return filesToLoad.map(f => JSON.parse(readFileSync(f, 'utf-8')) as ACIBenchResult);
}

// ---------------------------------------------------------------------------
// ACI-BENCH HTML generation
// ---------------------------------------------------------------------------

function generateACIBenchHTML(results: ACIBenchResult[]): string {
  const isComparison = results.length > 1;
  const latest = results[results.length - 1];
  const thresholds = latest.thresholds;
  ...
  const CATEGORY_COLORS: Record<string, { bg: string; border: string }> = {
    'SOAP Section ROUGE Scores': { bg: 'rgba(59, 130, 246, 0.75)', border: 'rgb(59, 130, 246)' },
  };

  const chartConfigs: string[] = [];
  let chartIndex = 0;

  for (const [category, metricKeys] of Object.entries(ACI_CATEGORIES)) {
    const availableKeys = metricKeys.filter(k => latest.metrics[k] !== undefined);
    if (availableKeys.length === 0) continue;

    const labels = availableKeys.map(k => ACI_METRIC_LABELS[k] || k);
    const canvasId = `chart-${chartIndex}`;
  ...
  const overallRouge = (latest.metrics.aci_overall_rouge * 100).toFixed(1);

  // Build metrics table rows
  const metricsTableRows = Object.entries(ACI_CATEGORIES).flatMap(([cat, keys]) =>
    keys.filter(k => latest.metrics[k] !== undefined).map(k => {
      const val = latest.metrics[k] ?? 0;
      const thresh = thresholds[k];
      const pass = thresh === undefined ? null : val >= thresh;
      const badge = pass === null ? '<span class="badge none">N/A</span>'
        ...
        : '<span class="badge fail">FAIL</span>';
      return `<tr>
                <td><strong>${ACI_METRIC_LABELS[k] || k}</strong></td>
                <td>${cat}</td>
                <td>${(val * 100).toFixed(1)}%</td>
                <td>${thresh !== undefined ? (thresh * 100).toFixed(0) + '%' : '\u2014'}</td>
                <td>${badge}</td>
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>ACI-BENCH \u2014 Ambient Clinical Intelligence Benchmark</title>
  <script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.7/dist/chart.umd.min.js"></script>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body {
...
<body>
  <div class="container">
    <h1>ACI-BENCH \u2014 Ambient Clinical Intelligence Benchmark</h1>
    <p class="subtitle">
      Model: ${latest.config.model || 'claude-sonnet-4-5-20250929'}
      &middot; ${new Date(latest.timestamp).toLocaleString()}
      &middot; ${latest.successCount} samples evaluated
    </p>

    <div class="stats-row">
      <div class="stat-card">
        <div class="label">Overall ROUGE</div>
        <div class="value ${latest.metrics.aci_overall_rouge >= 0.25 ? 'pass' : 'fail'}">${overallRouge}%</div>
      </div>
      <div class="stat-card">
        <div class="label">Samples OK</div>
        <div class="value pass">${latest.successCount}</div>
      </div>
      <div class="stat-card">
        <div class="label">Errors</div>
        <div class="value ${latest.errorCount > 0 ? 'fail' : 'pass'}">${latest.errorCount}</div>
      ...
      <div class="stat-card">
        ...
        <div class="value neutral">ROUGE-1</div>
      </div>
    </div>
    ...
    <div class="reference-card">
      <h2>Published Reference Results (ACI-BENCH, Nature Scientific Data 2023)</h2>
      <div class="reference-row">
        <span class="model">T5-large</span>
        <span class="score">~42 ROUGE-1</span>
      </div>
      <div class="reference-row">
        <span class="model">GPT-4</span>
        <span class="score">~46 ROUGE-1</span>
      </div>
      <div class="reference-row">
        <span class="model">This run (${latest.config.model || 'claude-sonnet-4-5-20250929'})</span>
        <span class="score highlight">${overallRouge} ROUGE-1</span>
      </div>
    </div>
    ...
    <div class="rerun-hint">
      <strong>Rerun ACI-BENCH:</strong><br>
      <code>npm run test:aci-bench</code> \u2014 full run (120 test samples)<br>
      <code>ACI_BENCH_SAMPLES=5 npm run test:aci-bench</code> \u2014 quick smoke test<br>
      <code>npm run test:aci-bench-visualize</code> \u2014 regenerate this chart<br>
      <code>npm run test:aci-bench-visualize -- --compare</code> \u2014 compare last 5 runs
    </div>

    <footer>
      Generated ${new Date().toLocaleString()} &middot; ACI-BENCH (Nature Scientific Data, 2023)
    </footer>
  </div>
...
const isACIBench = args.includes('--aci-bench');

if (isACIBench) {
  const aciResults = loadACIBenchResults();
  const html = generateACIBenchHTML(aciResults);
  const outputPath = join(RESULTS_DIR, 'aci-bench-chart.html');
  writeFileSync(outputPath, html);
  console.log(`ACI-BENCH chart saved to ${outputPath}`);
  try {
